                                ...
                                <?php foreach ($data['grupo'] as $obj): ?>
                                    <tr id="tr-<?= $obj->grupo_id ?>">
                                        <td id="td-pos"  class="text-center">
                                            <input type="number" class="form-control relpos"
                                                   name="relprod_pos" id="<?= $obj->relprod_id ?>"
                                                   style="width: 70px; text-align: center;"
                                                   value="<?= $obj->relprod_pos ?>" maxlength="999"/>
                                        </td>
                                        <td id="td-nome"><?= $obj->grupo_nome ?> - <?= $obj->grupo_desc ?></td>
                                        <td id="td-preco"><?= ($obj->grupo_tipo == 1) ? 'Obrigatório' : 'Opcional'; ?></td>
                                        <td class="text-center">
                                            <a data-cat="<?= $obj->relprod_categoria ?>"
                                               data-grp="<?= $obj->relprod_grupo ?>"
                                               title="Desvincular Grupo"
                                               data-toggle="tooltip"
                                               class="btn btn-sm btn-danger btn-rem-grupo">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <input type="hidden" name="categoria_id"
                                   value="<?php echo $data['categoria']->categoria_id; ?>"/>
...
<script type="text/javascript">
    ...
    $('.btn-rem-grupo').on('click', function () {
        ...
        $.post(url, {categoria_id: cat, grupo_id: grp}, function (rs) {
            window.location = rs;
        })
    });
    $('.relpos').on('change',function () {
        ...
        $.post(url,{relprod_id: id, relprod_pos: pos},function (rs) {
            _alert_success();
        })
    })
