            ...
            <div class="container-fluid" id="pcont">
                ...
                <div class="cl-mcont">
                    ...
                    <div class="block-flat">
                        <div class="header">							
                            <h3><?= $data['opcao']->categoria_nome ?> > <?= $data['opcao']->item_nome ?> > <?= $data['opcao']->opcao_nome ?> > <span class="text-primary">Editar Opção</span>
                                <span class="pull-right"><a href="<?= Http::base() ?>/opcao/item/<?= $data['opcao']->opcao_item ?>/" class="btn btn-primary"><i class="fa fa-chevron-circle-left"></i> Listar Opções</a></span>
                            </h3>
                        </div>
                        <div class="content">
                            <form action="<?= Http::base() ?>/opcao/gravar/" method="post" role="form" autocomplete="off" enctype="multipart/form-data"> 
                                <input type="hidden" name="opcao_id" id="opcao_id" value="<?= $data['opcao']->opcao_id ?>">                                        
                                <input type="hidden" name="opcao_item" id="opcao_item" value="<?= $data['opcao']->opcao_item ?>">                                        
                                <div class="row">

                                    <div class="col-md-6 col-xs-12">
                                        <div class="form-group">
                                            <label for="opcao_nome">Nome da opção <span class="text-danger">*</span></label> 
                                            <input type="text"  name="opcao_nome" id="opcao_nome" class="form-control" placeholder="Informe o nome da opção" 
                                                   value="<?= $data['opcao']->opcao_nome ?>" required>
                                        </div>    
                                    </div>    
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            <label for="opcao_preco">Preço <span class="text-danger">*</span></label> 
                                            <input type="text" name="opcao_preco" id="opcao_preco" class="form-control" placeholder="Informe o preço" 
                                                   value="<?= Filter::moeda($data['opcao']->opcao_preco) ?>" required>
                                        </div>    
                                    </div>    
                                </div>    
                                <div class="form-group">
                                    <label for="opcao_desc">Descrição</label> 
                                    <textarea name="opcao_desc" id="opcao_desc" class="form-control"> <?= $data['opcao']->opcao_desc ?> </textarea>
                                </div> 
                                <p class="text-center">
